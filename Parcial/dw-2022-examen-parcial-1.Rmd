...
SIETE: Si tratamos de hacer eso, le agregará un valor NULL ya que no existe (revisar)
...
```{r revision preliminar data}
glimpse(parcial_anonimo)
summary(parcial_anonimo)
```
...
```{r }
# = parcial_anonimo %>% select(Cliente, Pais) %>% summarise(Paises = 
#_distinct(Pais)) %>% filter(Paises > 1)
#clientes_pais

n_distinct(parcial_anonimo$Pais)

#parcial_anonimo %>% select(Cliente, Venta, Pais) %>% group_by(Cliente) %>% summarise(TotalV=sum(Venta
#)) %>% filter(TotalV>10) %>% arrange(desc(TotalV)) %>% hchart("column", 
#hcaes(x = Cliente, y = TotalV)) %>% hc_title(text = 
#"<b>Top 10 clientes rentables</b>")
```
